...
async function main() {
  ...
  console.log('正式排程已啟用（週四地選、週五天選；即時配對 22:00–01:00）：')
  for (const row of flags ?? []) console.log(`  ${row.key} = ${row.enabled}`)

  const [{ data: day }, { data: heavenOn }, { data: earthOn }, { data: instantOpen }] = await Promise.all([
    supabase.rpc('app_day_key_now'),
    supabase.rpc('fated_pair_heaven_active_for_day'),
    supabase.rpc('fated_pair_earth_active_for_day'),
    supabase.rpc('instant_match_open_now'),
  ])
  console.log(
    `app_day_key: ${day} | heaven_on: ${heavenOn} | earth_on: ${earthOn} | instant_open: ${instantOpen}`,
  )
}
