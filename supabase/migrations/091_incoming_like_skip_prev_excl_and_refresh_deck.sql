-- 091：探索 incoming 喜歡不受「昨日 6 人」排除 ＋ 套用時清空「當 app 日」快取 deck（強制下次進探索重建）
--
-- 前置：090 已套用。本 migration 依序在 089 探索邏輯上修補。
-- 081 的 v_prev_excl 仍用於「隨機補位」階段，但對「對我 like／super_like 的人」incoming 優先池不再套用。
--
-- 套用後：各帳號下次開探索會重新組 6 人；曾對你按愛心且符合區域／性別等條件者會優先進前段
-- （即使昨天曾出現在你 deck 裡）。不等 22:00 也可立即驗證（仍屬同一 app 日，但 deck 已重建）。
...
begin
  ...
  for v_pass in 1..2 loop
    ...
    loop
      ...
      -- 091：incoming 喜歡你的人可破例出現，即使昨天 deck 已有此人（v_prev_excl 僅限隨機補位）
...
-- 一次性：清空「目前 app 日」全站探索快取，等同強制所有人下次進探索重建名單
do $$
declare
  v_day text := public.app_day_key_now();
  v_deleted int;
begin
  delete from public.daily_discover_deck
  where app_day_key = v_day;

  get diagnostics v_deleted = row_count;
  raise notice '091: 已清除 app 日 % 的 daily_discover_deck % 列（下次探索 RPC 會重建）', v_day, v_deleted;
end;
$$;
