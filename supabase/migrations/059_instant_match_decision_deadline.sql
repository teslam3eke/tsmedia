-- 059：加好友決策視窗 2 分鐘（自 7 分鐘聊天結束起算）；逾時視為未互加。
-- instant_match_poll：回傳 user_a/user_b、both 決策未互加時改為 status=done；closed 不再卡在 in_session。
-- instant_session_decide：聊天結束後超過 2 分鐘不得再按。
