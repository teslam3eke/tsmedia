-- ============================================================
-- 041: 超級喜歡立即建立配對（與互相喜歡相同 matches／聊天／Realtime 彈窗）
--
-- 先前：super_like 只發 app_notifications「有人對你按了超級喜歡」，matches 仍須對方
--       曾 like/super_like 你才會建立 → 無法開聊天；收件者只在「我的」看到通知。
-- 現在：super_like 成功即 insert matches（對方有 uuid）；不再寫 super_like_received；
--       與一般配對相同由前端 subscribe matches INSERT 顯示 MatchSuccessSplash。
-- ============================================================
...
begin
  ...
  -- 配對：① 對方早已 like/super_like 你 → 互相配對
  --       ② 本次為 super_like 且對方有 uuid → 立即配對（不需對方先按喜歡）
