-- 152：探索 deck — 需重複時先跨區，再回期望區同區重複
--
-- 102 已在「新面孔」階段跨區補位，但 148 在加權重複時仍先掃期望區（④）再掃其他區（⑤）。
-- 期望區池子用盡時，會連日從北部少數人輪替，其他區域候選進不了 deck。
-- 修正：②③ 不變；④ 改為其他區加權重複；⑤ 改為期望區加權重複（最後手段）。
-- 清空當 app 日快取 deck，讓已部署環境當日即依新順序重建。
...
begin
  ...
  for v_pass in 1..2 loop
    ...
    -- ④ 其他區域：加權重複（152：跨區優先，期望區新面孔用盡後先補其他區）
    if cardinality(v_picked) < 6 and cardinality(v_order_other) > 0 then
      foreach r in array v_order_other loop
        exit when cardinality(v_picked) >= 6;
        v_need := 6 - cardinality(v_picked);
...
comment on function public.__get_daily_discover_deck_impl() is
  '探索 deck：新面孔仍期望區優先；加權重複時先跨區再回期望區（152）。';
