-- ============================================================
-- Migration 030: 愛心／超喜不以「當 app 日」為限
-- 1) record_profile_interaction：改為「曾對該 target 送過 like/super_like」
--    即不可再扣點（不限日期）；配對仍看對方是否曾對你有 like/super_like（本即不限日）。
-- 2) _daily_discover_profiles_json：liked_today / super_liked_today 改為不限
--    interaction_app_day_key（JSON 鍵名維持，供前端相容；語意為「曾送過」）。
-- ============================================================
