-- ============================================================
-- 077：即時配對僅在台灣時間 22:00–01:00 可加入等候／撮合
-- （進行中場次不受限；p_enqueue:false 仍可查狀態）
-- ============================================================
...
as $$
  select
    extract(hour from (current_timestamp at time zone 'Asia/Taipei'))::int >= 22
...
grant execute on function public.instant_match_open_now() to authenticated;
