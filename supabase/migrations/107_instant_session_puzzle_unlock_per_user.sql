-- ============================================================
-- 107：即時房拼圖道具解鎖改為「每人各自進度」（不再寫入 instant_sessions 共用欄位）
-- ============================================================

create table if not exists public.instant_session_puzzle_unlocks (
...
comment on table public.instant_session_puzzle_unlocks is
  '即時七分鐘房：各使用者以道具解鎖的拼圖格（0–47）；僅本人可見';

alter table public.instant_session_puzzle_unlocks enable row level security;

drop policy if exists "instant_puzzle_unlock: own read" on public.instant_session_puzzle_unlocks;
create policy "instant_puzzle_unlock: own read"
  on public.instant_session_puzzle_unlocks for select
  using (auth.uid() = user_id);

-- 舊版 106 共用欄位不再使用（若存在則移除）
alter table public.instant_sessions
  drop column if exists puzzle_manual_unlocked_tiles;

-- ─── 查詢本人即時房道具解鎖格 ───────────────────────────────────────────────

create or replace function public.get_my_instant_session_puzzle_unlocked_tiles(p_session_id uuid)
returns int[]
language plpgsql
security definer
...
begin
  ...
  select unlocked_tiles into v_tiles
  from public.instant_session_puzzle_unlocks
  where session_id = p_session_id
    and user_id = v_user;

  return coalesce(v_tiles, '{}'::int[]);
end;
$$;

grant execute on function public.get_my_instant_session_puzzle_unlocked_tiles(uuid) to authenticated;

-- ─── 即時房消耗 blur_unlock（僅更新本人列）──────────────────────────────────
...
-- ─── instant_session_decide：升格時不再合併共用道具格至 photo_unlock_states ───
-- （各使用者進度留於 instant_session_puzzle_unlocks；配對聊天仍依 064 空狀態＋訊息推導）
...
do $$
begin
  alter publication supabase_realtime add table public.instant_session_puzzle_unlocks;
