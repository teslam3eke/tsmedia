-- ============================================================
-- 096：月費入帳 — 先延長 subscription，開通禮失敗不 rollback 訂閱
-- （道具包僅一筆 credit insert；開通禮三筆較易因邊角失敗拖垮整筆 grant）
-- ============================================================
...
begin
  ...
  if v_welcome is null then
    ...
    exception
      when others then
        raise warning 'grant_monthly_membership welcome gifts failed for %: %', p_user_id, sqlerrm;
    end;
  end if;
