-- ============================================================
-- 081: 探索每日牌組 — 硬性排除「前一 app 日」牌組中的 6 人
--
-- 060 僅避免與昨日「同序」；本 migration 改為同一 viewer 今日 deck
-- 不得包含昨日 daily_discover_deck 內任一 target（各 phase 皆過濾）。
-- 候選池極小時可能少於 6 人（寧可不足，也不重複昨日名單）。
-- ============================================================
...
begin
  ...
  loop
    ...
    if not (incoming_uid = any(v_picked))
       and not (incoming_uid = any(v_prev_deck)) then
  ...
  if cardinality(v_picked) > 1 then
    ...
    loop
      ...
      exit when cardinality(coalesce(v_prev_deck, array[]::uuid[])) = 0;
