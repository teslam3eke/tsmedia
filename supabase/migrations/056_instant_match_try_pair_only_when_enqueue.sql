-- 056：僅在 p_enqueue=true（使用者表達排隊意圖）時執行 _instant_try_pair_locked。
-- 否則 instant_match_poll(false) 若第一段未 early-return（或搭配幽靈佇列列），
-- 可能一路執行到底並撮合——與「我知道了／僅查狀態」預期不符（使用者以為未按排隊卻進配對）。
