-- 148：探索年齡偏好與候選人不足時的漸進放寬。
-- 已存在且非空的當日 deck 不重建，偏好於下一次建立 deck 時生效。
...
alter table public.profiles
  add constraint profiles_preferred_age_min_check
    check (preferred_age_min is null or preferred_age_min between 18 and 80),
  add constraint profiles_preferred_age_max_check
    check (preferred_age_max is null or preferred_age_max between 18 and 80),
  add constraint profiles_preferred_age_pair_check
    check ((preferred_age_min is null) = (preferred_age_max is null)),
  add constraint profiles_preferred_age_range_check
    check (
      preferred_age_min is null
      or preferred_age_max - preferred_age_min >= 10
    );

comment on column public.profiles.preferred_age_min is
  '探索偏好年齡下限；null 時依本人年齡計算有效預設範圍。';

comment on column public.profiles.preferred_age_max is
  '探索偏好年齡上限；null 時依本人年齡計算有效預設範圍。';

-- 保留既有 _daily_discover_candidate_ok signature 與所有呼叫者；
-- deck 建立流程先決定一次有效上下限，再由本 helper 追加年齡條件。
create or replace function public._daily_discover_candidate_ok_age(
  p_viewer uuid,
  p_target uuid,
  p_my_gender text,
  p_region text,
  p_exclude_shown boolean,
  p_age_min integer,
  p_age_max integer
)
returns boolean
...
as $$
  select
    public._daily_discover_candidate_ok(
      p_viewer,
      p_target,
      p_my_gender,
      p_region,
...
comment on function public._daily_discover_candidate_ok_age(
  uuid, uuid, text, text, boolean, integer, integer
) is
  '探索 deck 候選：沿用既有硬性條件，並套用本次建牌已決定的年齡上下限；上下限皆為 null 時不限制年齡。';
...
declare
  ...
  v_pref_norm text;
  v_my_age integer;
  v_saved_age_min integer;
  v_saved_age_max integer;
  v_strict_age_min integer;
  v_strict_age_max integer;
  v_age_min integer;
  v_age_max integer;
  v_hard_candidate_count integer;
  v_order_expand text[];
  v_order_pref text[];
  v_order_other text[];
...
begin
  ...
  -- null 偏好採本人年齡 ±5；靠近 18／80 時向另一側補足 10 歲。
  if v_saved_age_min is not null and v_saved_age_max is not null then
    v_strict_age_min := v_saved_age_min;
    v_strict_age_max := v_saved_age_max;
  elsif v_my_age is null then
    v_strict_age_min := 18;
    v_strict_age_max := 80;
  else
    v_my_age := greatest(18, least(80, v_my_age));
    v_strict_age_min := greatest(18, v_my_age - 5);
    v_strict_age_max := least(80, v_my_age + 5);

    if v_strict_age_max - v_strict_age_min < 10 then
      if v_strict_age_min = 18 then
        v_strict_age_max := 28;
      else
        v_strict_age_min := 70;
      end if;
  ...
  v_age_min := v_strict_age_min;
  v_age_max := v_strict_age_max;

  -- 僅在每次新建 deck 時做至多三次池量檢查，不在逐一挑選候選人時重算。
  select count(*)
  into v_hard_candidate_count
  from public.profiles p
  where public._daily_discover_candidate_ok_age(
    v_viewer, p.id, v_my_gender, null::text, false, v_age_min, v_age_max
  );

  if v_hard_candidate_count < 6 then
    v_age_min := greatest(18, v_strict_age_min - 3);
    v_age_max := least(80, v_strict_age_max + 3);
  ...
  if v_hard_candidate_count < 6 then
    v_age_min := greatest(18, v_strict_age_min - 6);
    v_age_max := least(80, v_strict_age_max + 6);
  ...
  if cardinality(v_prev_raw) > 0 then
    ...
    where public._daily_discover_candidate_ok_age(
      v_viewer, u.uid, v_my_gender, null::text, false, v_age_min, v_age_max
    );
  else
  ...
  for v_pass in 1..2 loop
    ...
    for incoming_uid in
      ...
      where exists (
        ...
        where public._daily_discover_candidate_ok_age(
          v_viewer, q.actor_uid, v_my_gender, ord.region, false, v_age_min, v_age_max
        )
      )
      order by
        case when p_in.age between v_strict_age_min and v_strict_age_max then 0 else 1 end,
        q.mx desc,
    ...
    foreach r in array v_order_pref loop
      ...
      for r_pick in
        ...
        from public.profiles p
        where public._daily_discover_candidate_ok_age(
            v_viewer, p.id, v_my_gender, r, true, v_age_min, v_age_max
          )
          and not (p.id = any(v_picked))
          and not (p.id = any(v_prev_excl))
        order by
          case when p.age between v_strict_age_min and v_strict_age_max then 0 else 1 end,
    ...
    if cardinality(v_picked) < 6 then
      foreach r in array v_order_pref loop
        ...
        for r_pick in
          ...
          where public._daily_discover_candidate_ok_age(
              v_viewer, p.id, v_my_gender, r, false, v_age_min, v_age_max
            )
            and not (p.id = any(v_picked))
            and not (p.id = any(v_prev_excl))
          order by
    ...
    if cardinality(v_picked) = 0 and not v_strict_pref then
      ...
      for r_pick in
        ...
        from public.profiles p
        where public._daily_discover_candidate_ok_age(
            v_viewer, p.id, v_my_gender, null::text, true, v_age_min, v_age_max
          )
          and not (p.id = any(v_picked))
          and not (p.id = any(v_prev_excl))
        order by
  ...
  -- 直接超喜維持既有額外追加槽位，不套用 base deck 年齡限制。
...
comment on function public.__get_daily_discover_deck_impl() is
  '探索 deck：保留 102 的區域、incoming like、昨日排除、shown、加權重複、洗牌與超喜追加，base 6 另套用漸進放寬的年齡偏好。';
