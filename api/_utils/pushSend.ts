...
/**
 * 聊天室每 15 秒更新一次心跳；45 秒內才視為仍在前景。
 * 視窗被強制關閉、pagehide 未送達時也會快速失效，避免殘留 presence 長時間吞推播。
 */
const CHAT_PRESENCE_TTL_MS = 45_000
...
/** 僅略過「此裝置目前正在看同一聊天室」的推播；事件不會送到 APNs，因此不構成 iOS silent push。 */
async function resolveChatPresenceSkipClientKeys(
  ...
  const { data, error } = await supabase
    ...
    .select('client_key, active_match_id, updated_at')
    .eq('user_id', userId)
    .eq('visibility', 'visible')
  ...
  for (const row of data ?? []) {
    const key = typeof row.client_key === 'string' ? row.client_key.trim() : ''
    if (!key || compactMatchId(String(row.active_match_id ?? '')) !== target) continue
    const updatedAt = row.updated_at ? Date.parse(String(row.updated_at)) : NaN
    /** 無效或過期的時間一律不略過，安全側選擇仍發推播。 */
...
/** 聊天訊息：同裝置正在看同一聊天室時不送；其他聊天室、背景與其他裝置照常推播。 */
