...
function discoverChatPuzzleIntroStorageKey(userId: string) {
  ...
  return `tsm-discover-chat-puzzle-intro:v5:${userId}`
}
            ...
            >
              ...
              <div className="mt-3 space-y-3 text-sm leading-relaxed text-slate-600">
                ...
                <p className="text-[13px] text-slate-500">
                  提醒：有效 VIP 會員每晚 10 點換日可領
                  <span className="mx-0.5 font-semibold text-slate-700">3 顆愛心</span>
                  與
...
function FeedbackCouponOfferBody() {
  const steps = [
    ...
    {
      key: 'follow',
      title: <>點擊 <strong className="font-black text-red-600">「追蹤」</strong></>,
      detail: <span>掌握最新活動消息</span>,
    },
    {
      key: 'feedback',
      title: <>私訊 <strong className="font-black text-red-600">真實意見</strong></>,
      detail: <span>告訴我們可以改進的地方</span>,
    },
    {
      key: 'completed',
      title: <>私訊 <strong className="font-black text-red-600">「已填寫問卷」</strong></>,
  ...
  return (
    <div className="mt-3 text-left text-[11px] leading-relaxed text-[#745a35]">
      ...
      <div className="mt-3 rounded-xl border border-amber-200 bg-white/70 px-3 py-2.5 text-left">
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      if (parts.length > 0) {
        setRewardFlash({
          variant: 'daily',
          title: claim.ok ? '今日獎勵已入帳' : '會員獎勵已更新',
          subtitle: parts.join(' · '),
        })
      }
    })()
